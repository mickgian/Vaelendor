...
def main():
    ...
    prev_checkpoint = read_file_safe(prev_checkpoint_path) if chapter_num > 1 else None

    # Read existing checkpoint
    existing_checkpoint = read_file_safe(checkpoint_path)

    print(f"\n=== CHECKPOINT GENERATOR — Capitolo {chapter_num:02d} ===\n")
    ...
    if prev_checkpoint:
        ...
        if chapter_num > 1:
            print(f"⚠️  Checkpoint precedente (dopo-capitolo-{chapter_num-1:02d}.md) non trovato!")

    print()

    # Party members for this chapter
    members = get_party_members(chapter_num)
    print(f"👥 Membri del party al Capitolo {chapter_num}: {', '.join(members)}")
    print()
    ...
    if existing_checkpoint:
        ...
        print()
        # Generate template
        template = f"""# Checkpoint — Dopo Capitolo {chapter_num:02d}
